// ===========================================================
// AST type models
import { cond, map, pipe, zipWith } from "ramda";
...
;;         |  ( cond <cond-clauses>)+(<else>)   /cond exp(cond clauses: List(cond-clause), else: cond-clause)
;;         |  ( <cexp> <cexp>* )            / AppExp(operator:CExp, operands:CExp[]))
;; <binding>  ::= ( <var> <cexp> )           / Binding(var:VarDecl, val:Cexp)
;; <cond-clause> ::= (<cexp><cexp>+) | (else <cexp>+)
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | Cond;
...
export type Cond = {tag: "Cond"; val: CondClause[]; else: CExp; }
...
export const makeLitExp = (val: SExpValue): LitExp =>
    ({tag: "LitExp", val: val});
export const makeCond = (val: CondClause[], elseClause: CExp): Cond =>
    ({tag: "Cond", val: val, else: elseClause});
...
export const isCond = (x: any): x is Cond => x.tag === "Cond";
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isCond(x);
...
const parseCondClause = (params: Sexp): Result<CondClause> => {
    if (isArray(params)) {
        let test = parseL31CExp(params[0]);
        let then = parseL31CExp(params[1]);
        if(isOk(test) && isOk(then))
            return makeOk(makeCondClause(test.value, then.value));
        else {
            return makeFailure("test or then not ok");
        }
    } else {
        return makeFailure("params is not an array");
    }    
}
    
const parseCondExp = (params: Sexp[]): Result<Cond> => 
    params.length < 2? makeFailure("missing case"):
    params[params.length-1][0] != "else"? makeFailure("missing else"):
    mapv(mapResult(parseCondClause, params) ,(cexps: CondClause[]) => 
        makeCond(cexps.slice(0,-1), cexps[cexps.length-1].then));
...
const unparseCondClause = (cc: CondClause) : string =>
    `(${unparseL31(cc.test)} ${unparseL31(cc.then)})`

const unparseCondExp = (c: Cond) : string =>
    `(cond ${map(unparseCondClause, c.val).join(" ")} (else ${unparseL31(c.else)}))`
